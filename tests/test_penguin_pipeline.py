...
@unittest.skipIf(tf.__version__ < "2", "Uses Flax Model only compatible with TF 2.x")
class PenguinPipelineLocalEndToEndTest(tf.test.TestCase, parameterized.TestCase):
    def setUp(self):
        super().setUp()
        ...
        raw_data_path = make_dataset.get_raw_data(
            pipeline_name=self._pipeline_name, project_dir=Path(self._test_dir)
        )
        processed_data_path = str(make_dataset.preprocess_data(raw_data_path))
        print(f"PROCESSED DATA PATH: {processed_data_path}")

        self._schema_path = os.path.join(
            os.path.dirname(__file__), "..", "schema", "user_provided", "schema.pbtxt"
        )
        self._data_root = os.path.join(self._test_dir, "data", self._pipeline_name)
        ...
        #   - day3
        #     - penguins_processed.csv
        self._data_root_span = os.path.join(
            self._test_dir, "test_data", self._pipeline_name
        )
        for day in ["day1", "day2", "day3"]:
            dst_dir = os.path.join(self._data_root_span, "labelled", day)
            fileio.makedirs(dst_dir)
            src_pth = processed_data_path
            dst_pth = os.path.join(dst_dir, "penguins_processed.csv")
            fileio.copy(src_pth, dst_pth)

        self._serving_model_dir = os.path.join(self._test_dir, "serving_model")
        self._pipeline_root = os.path.join(
            self._test_dir, "tfx", "pipelines", self._pipeline_name
        )
        self._metadata_path = os.path.join(
            self._test_dir, "tfx", "metadata", self._pipeline_name, "metadata.db"
        )
        self._module_file = os.path.join(
            os.path.dirname(__file__), "..", "spolm", "features", "build_features.py"
        )
    ...
    def _assertExecutedOnce(self, component: str) -> None:
        ...
        execution_path = os.path.join(component_path, ".system", "executor_execution")
        execution = fileio.listdir(execution_path)
        self.assertLen(execution, 1)

    def _assertPipelineExecution(
        self,
    ...
    def testPenguinPipelineLocal(self):
        ...
        logging.info(
            "Starting the second pipeline run. All components except "
        ...
        logging.info(
            "Starting the third pipeline run. "
    ...
    @unittest.skip(reason="Enable this test when Flax supports tuning")
    ...
    # TODO: This test currently fails with
    # RuntimeError: tensorflow.python.framework.errors_impl.InvalidArgumentError:
    # Name: <unknown>, Feature: body_mass_g (data type: float) is required but could not
    # be found. [[{{node ParseExample/ParseExampleV2}}]] [while running
    # 'RunInference[train]/RunInference/RunInferenceImpl/Predict/Predict']
    @unittest.skip(reason="Skip")
    def testPenguinPipelineLocalWithBulkInferrer(self):
        LocalDagRunner().run(
            penguin_pipeline_local._create_pipeline(
                pipeline_name=self._pipeline_name,
                data_root=self._data_root,
    ...
    def _get_input_examples_artifacts(
        self, store: mlmd.MetadataStore, execution_type: str
    ...
    def testPenguinPipelineLocalWithRollingWindow(self):
        examplegen_input_config = proto.Input(
            splits=[
        ...
        examplegen_range_config = proto.RangeConfig(
            static_range=proto.StaticRange(start_span_number=1, end_span_number=1)
        )
        ...
        self.assertEqual(
            1,
            tft_input_examples_artifacts[0]
            .custom_properties[_SPAN_PROPERTY_NAME]
            .int_value,
        )

        # Trigger the pipeline for the second span.
        examplegen_range_config = proto.RangeConfig(
            static_range=proto.StaticRange(start_span_number=2, end_span_number=2)
        )
        run_pipeline(examplegen_range_config)
        ...
        spans = {
            ...
            .custom_properties[_SPAN_PROPERTY_NAME]
            .int_value,
            tft_input_examples_artifacts[1]
            .custom_properties[_SPAN_PROPERTY_NAME]
            .int_value,
        }
        ...
        self.assertLen(
            self._get_input_examples_artifacts(store, trainer_execution_type), 2
        )

        # Trigger the pipeline for the thrid span.
        examplegen_range_config = proto.RangeConfig(
            static_range=proto.StaticRange(start_span_number=3, end_span_number=3)
        )
        run_pipeline(examplegen_range_config)
