...
The utilities in this file are used to build a model with Flax.
...
FEATURE_KEYS = FEATURES["penguin"]


# The transformed feature names
_FEATURE_KEYS_XF = list(map(transformed_name, FEATURE_KEYS))
...
_Array = np.ndarray
_InputBatch = Dict[str, _Array]  # keys are _FEATURE_KEYS_XF and values f32[B, 1]
...
def _make_trained_model(
    ...
    init_val = {
        feature: jnp.array([[1.0]], dtype=jnp.float32) for feature in _FEATURE_KEYS_XF
    }
    ...
    for epoch in range(1, num_epochs + 1):
        optimizer, train_metrics = _train_epoch(
            model, optimizer, train_data, steps_per_epoch
        )
        absl.logging.info(
            "Flax train epoch: %d, loss: %.4f, accuracy: %.2f",
            epoch,
            train_metrics["loss"],
        ...
        eval_metrics = _eval_epoch(
            model, optimizer.target, eval_data, eval_steps_per_epoch
        )
        absl.logging.info(
            "Flax eval epoch: %d, loss: %.4f, accuracy: %.2f",
            epoch,
            eval_metrics["loss"],
            eval_metrics["accuracy"] * 100,
        )
        summary_writer.scalar("epoch_train_loss", train_metrics["loss"], epoch)
        summary_writer.scalar("epoch_train_accuracy", train_metrics["accuracy"], epoch)
        summary_writer.scalar("epoch_eval_loss", eval_metrics["loss"], epoch)
        summary_writer.scalar("epoch_eval_accuracy", eval_metrics["accuracy"], epoch)
...
def _train_epoch(
    model: _FlaxPenguinModel,
    optimizer: flax.optim.OptimizerDef,
    train_data: tf.data.Dataset,
    steps_per_epoch: int,
):
...
@functools.partial(jax.jit, static_argnums=0)
def _eval_step(
    model: _FlaxPenguinModel, params: _Params, inputs: _InputBatch, labels: _LabelBatch
):
    logits = model.apply({"params": params}, inputs)
...
def _mean_epoch_metrics(
    ...
    return {
        metric_name: np.mean([metrics[metric_name] for metrics in batch_metrics_np])
